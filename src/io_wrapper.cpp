...
void export_ma_obj_only(const std::string& maname, const MedialMesh& mat) {
  ...
  std::string ma_path = folder_name + "mat_" + maname + "_" + get_timestamp() + ".ma";
  export_ma_given(ma_path, vertices, edges, faces, true /*is_use_given_name*/);
}
...
/*
 * Export medial mesh in .ma format with MedialType appended to each vertex line.
 * Format:
 * numVertices numEdges numFaces
 * v x y z r <MedialType>
 * e v1 v2
 * f v1 v2 v3
 *
 * MedialType: MUNKOWN=-1, SHEET=0, SEAM=1, BOUNDARY=2, JUNCTION=3
 */
void export_ma_with_type(const std::string& maname, const MedialMesh& mat) {
  std::string folder_name = g_output_dir + "/" + maname + "/mat/";
  create_dir(folder_name);
  ...
  // Build per-vertex MedialType using priority: JUNCTION > SEAM > BOUNDARY > SHEET
  std::vector<int> vertex_types(vertices.size(), MedialType::MUNKOWN);

  for (const auto& mstruct : mat.mstructure) {
    switch (mstruct.type) {
      case MedialType::SHEET:
        for (const auto& mfid : mstruct.m_face_ids) {
          const auto& mface = mat.faces.at(mfid);
          for (int j = 0; j < 3; j++) {
            auto it = map_matv.find(mface.vertices_[j]);
            if (it != map_matv.end()) {
              int nv = it->second;
              if (vertex_types[nv] == MedialType::MUNKOWN)
                vertex_types[nv] = MedialType::SHEET;
            }
          }
        }
        break;
      ...
      case MedialType::BOUNDARY:
        for (const auto& meid : mstruct.m_edge_ids) {
          const auto& medge = mat.edges.at(meid);
          for (int j = 0; j < 2; j++) {
            auto it = map_matv.find(medge.vertices_[j]);
            if (it != map_matv.end()) {
              int nv = it->second;
              // SEAM/BOUNDARY override SHEET but not each other or JUNCTION
              if (vertex_types[nv] == MedialType::MUNKOWN ||
                  vertex_types[nv] == MedialType::SHEET)
                vertex_types[nv] = mstruct.type;
            }
          }
        }
        break;
      case MedialType::JUNCTION:
        for (const auto& mvid : mstruct.m_sphere_ids) {
          auto it = map_matv.find(mvid);
          if (it != map_matv.end())
            vertex_types[it->second] = MedialType::JUNCTION;
        }
        break;
      default:
        break;
  ...
  std::string mat_path = folder_name + "mat_typed_" + maname + ".mat";
  std::ofstream fout(mat_path);
  if (!fout.is_open()) {
    std::cerr << "Error: Unable to open file for writing: " << mat_path << std::endl;
    return;
  }

  fout << vertices.size() << " " << edges.size() << " " << faces.size() << "\n";
