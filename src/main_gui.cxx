...
void MainGuiWindow::auto_opt_rpd_only_particle() {
  ...
  export_ma_with_type(instance_->name_no_ext, *(instance_->mmesh));
}
...
void MainGuiWindow::show_tet_mesh(const TetMesh& tet_mesh, bool is_shown_meta) {
  ...
  for (const auto& one_ce_line : tet_mesh.ce_lines) {
    for (const auto& one_ce_id : one_ce_line.fe_ids) {
      const auto one_ce = tet_mesh.feature_edges.at(one_ce_id);
      ce_edge_vs.push_back(one_ce.t2vs_pos[0]);
      ce_edge_vs.push_back(one_ce.t2vs_pos[1]);
      ce_edges.push_back({{ce_edge_vs.size() - 2, ce_edge_vs.size() - 1}});
      ce_fl_ids.push_back(one_ce_line.id);
      ce_fe_ids.push_back(one_ce.id);
    }
  }
#ifdef MATSTRUCT_WITH_VISUALIZATION
  auto my_ce_edges =
  ...
  for (const auto& one_se_line : tet_mesh.se_lines) {
    for (const auto& one_se_id : one_se_line.fe_ids) {
      const auto one_se = tet_mesh.feature_edges.at(one_se_id);
      se_edge_vs.push_back(one_se.t2vs_pos[0]);
      se_edge_vs.push_back(one_se.t2vs_pos[1]);
      se_edges.push_back({{se_edge_vs.size() - 2, se_edge_vs.size() - 1}});
      se_fl_ids.push_back(one_se_line.id);
      se_fe_ids.push_back(one_se.id);
    }
  }
#ifdef MATSTRUCT_WITH_VISUALIZATION
  auto my_se_edges =
